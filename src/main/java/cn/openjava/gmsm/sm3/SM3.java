package cn.openjava.gmsm.sm3;

import cn.openjava.gmsm.utils.ConvertUtil;
...
public class SM3 {
    ...
    private static byte[] bigEndianIntToByte(int num) {
        return back(ConvertUtil.intToBytes(num));
    }

    private static int bigEndianByteToInt(byte[] bytes) {
        return ConvertUtil.byteToInt(back(bytes));
    }
    ...
    public static byte[] padding(byte[] in, int bLen) {
        ...
        pos += padd.length;
        byte[] tmp = back(ConvertUtil.longToBytes(n));
